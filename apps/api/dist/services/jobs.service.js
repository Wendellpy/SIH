...
class JobsService {
    ...
    createJob(type, filename, fileSizeBytes, mimeType) {
        const job = {
            id: `job-${crypto_1.default.randomUUID().slice(0, 8)}`,
            type,
            status: 'PENDING',
            progress: 0,
        ...
        store_js_1.db.saveJob(job);
        this.notify(job);
        return job;
    }
    async processFloorplanJob(jobId, buildingName, floorNumber, baseUlpin) {
        const job = store_js_1.db.getJob(jobId);
        if (!job)
            return;
        const stages = [
        ...
        generatedUnits.forEach(u => {
            store_js_1.db.addVerticalUnit({
                id: `unit-ai-${crypto_1.default.randomUUID().slice(0, 8)}`,
    ...
    async processLidarJob(jobId, baseUlpin) {
        const job = store_js_1.db.getJob(jobId);
        if (!job)
            return;
        const stages = [
