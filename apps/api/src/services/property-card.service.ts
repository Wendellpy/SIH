...
export class PropertyCardService {
   ...
   */
  static generateRecordHash(record: any): string {
  ...
  static async generateCard(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const qrCodeDataUrl = await QRCode.toDataURL(liveUrl, { margin: 1, width: 120 });
        ...
        if (type === 'VerticalUnit') {
          const unit = entity as VerticalUnit;
