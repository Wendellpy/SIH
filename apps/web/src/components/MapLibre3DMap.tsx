...
export const MapLibre3DMap: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      map.on('load', () => {
        setMapLoaded(true);
        console.log('[MapLibre] 3D Vector Map loaded successfully with 45-degree pitch');
      });
      ...
      map.on('click', '3d-buildings', async (e) => {
        ...
        const baseUlpin = `MH13BOM${Math.floor(10000000 + Math.random() * 90000000)}`;
  ...
  return (
    <div className="relative w-full h-full min-h-[600px] rounded-2xl overflow-hidden border border-white/10 shadow-2xl bg-[#070b14] flex flex-col">
      ...
      {/* Top-Left HUD */}
      <div className="absolute top-4 left-4 glass-panel rounded-xl p-3.5 border border-white/10 max-w-sm pointer-events-auto shadow-2xl space-y-2.5">
        ...
        <div className="space-y-1">
          ...
          <div className="grid grid-cols-2 gap-1.5 font-mono text-[10px]">
            <button
              ...
              className="px-2 py-1 bg-surface-100 hover:bg-surface-200 text-slate-200 rounded text-left truncate border border-white/5 transition-all font-bold text-cyan-300"
            >
              🏙️ Worli Supertalls
            </button>
            <button
            ...
            <button
              onClick={() => flyToDistrict(72.8688, 19.0607, 15.2, 45, -20)}
              className="px-2 py-1 bg-surface-100 hover:bg-surface-200 text-slate-200 rounded text-left truncate border border-white/5 transition-all"
            >
              🏢 BKC Financial Hub
            </button>
      ...
      {selectedBuildingInfo && (
        <div className="absolute bottom-4 right-4 glass-panel-glow rounded-2xl p-4 border border-brand-primary/50 max-w-sm pointer-events-auto space-y-3 shadow-2xl animate-fade-in overflow-y-auto max-h-[80vh]">
          ...
          {selectedBuildingInfo.ownership && (
            <div className="glass-card p-2.5 rounded-xl border border-amber-500/20 bg-amber-950/10 space-y-1.5">
          ...
          <div className="glass-card p-2.5 rounded-xl border border-white/10 bg-black/40 space-y-1">
          ...
          <button
            ...
            className="w-full py-2 px-3 rounded-xl bg-gradient-to-r from-brand-primary to-indigo-600 hover:from-cyan-400 hover:to-indigo-500 text-white text-xs font-semibold flex items-center justify-center gap-1.5 shadow-neon-cyan transition-all"
          >
